...
export type HeaderNavModule = 'rankings' | 'pricing' | 'checkQuota'
...
const DEFAULT_HEADER_NAV_MODULES: HeaderNavModules = {
  ...
  checkQuota: { enabled: true, requireAuth: false },
  docs: true,
  about: true,
}
...
const DEFAULTS: Record<HeaderNavModule, ModuleAccess> = {
  ...
  checkQuota: DEFAULT_HEADER_NAV_MODULES.checkQuota,
}
...
export function parseHeaderNavModules(raw: unknown): HeaderNavModules {
  ...
  Object.entries(parsed).forEach(([key, value]) => {
    ...
    if (key === 'checkQuota') {
      result.checkQuota = parseAccess(value, result.checkQuota)
      return
    }
